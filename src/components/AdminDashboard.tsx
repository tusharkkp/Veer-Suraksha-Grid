...
const AdminDashboard: React.FC = () => {
  const { t } = useLanguage();
  const { activeEmergency, assets, workers, machines, telemetryMap } = useSystem();
  ...
  const liveDevices = Object.values(telemetryMap).filter(t => t.isLive).length;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-5 gap-3 p-4 pb-0">
        {[
          { label: t('nav.workers'), value: `${activeWorkers}/${workers.length}`, sub: t('status.active'), color: 'text-[hsl(var(--risk-green))]' },
          ...
          { label: 'ESP32 Live', value: liveDevices.toString(), sub: 'Firebase', color: liveDevices > 0 ? 'text-[hsl(var(--risk-green))]' : 'text-muted-foreground' },
        ].map((s, i) => (
